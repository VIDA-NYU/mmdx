...
def test_add_and_query_labels():
    db_path = "data/test/db_add_and_query_labels/"
    model = RandomMockModel()
    db = VectorDB.from_data_path(data_path, db_path, model)
    assert db != None
    ...
    # initially, images should have zero labels
    labels = db.get_labels(image_path=random_query_path)
    assert labels == []
    assert len(labels) == 0

    # when we add one label, get_labels must return it
    db.add_label(image_path=random_query_path, label="label_1")
    assert db.get_labels(image_path=random_query_path) == ["label_1"]
    assert db.get_labels(image_path=random_query_path_2) == []

    # when we add another label, get_labels must return all of them
    db.add_label(image_path=random_query_path, label="label_2")
    assert db.get_labels(image_path=random_query_path) == ["label_1", "label_2"]
    assert db.get_labels(image_path=random_query_path_2) == []
    ...
    search_result = hits[hits["image_path"] == random_query_path].copy().reset_index()
    assert len(search_result.index) == 1
    assert search_result.loc[0]["image_path"] == random_query_path
    ...
    # items that are not labels must return an empty array
    search_result = hits[hits["image_path"] != random_query_path].copy().reset_index()
